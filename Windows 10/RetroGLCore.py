...
core_dir = "/home/ralt/Downloads/RetroArch_cores"
info_dir = "/home/ralt/Downloads/libretro-core-info"  # Directory where .info files are stored
retroarch_cfg_path = "/home/ralt/Downloads/retroarch.cfg"  # Path to the retroarch.cfg file
log_dir = "/home/ralt/Downloads"
savestate_dir = "/home/ralt/Downloads/RetroArch-Linux-x86_64/RetroArch-Linux-x86_64.AppImage.home/.config/retroarch/states"  # Base directory where save states are located
screenshot_dir = "/home/ralt/Downloads/RetroArch-Linux-x86_64/RetroArch-Linux-x86_64.AppImage.home/.config/retroarch/screenshots"  # Directory where screenshots are saved
bios_dir = "/home/ralt/Downloads/RetroArch_cores/RetroArch-Linux-x86_64/RetroArch-Linux-x86_64.AppImage.home/.config/retroarch/system"  # Directory for BIOS files
shader_dir = "/home/ralt/Downloads/shaders_slang"
default_shader_dir = shader_dir  # Initialize default_shader_dir with the initial value of shader_dir
shader_extensions = ('.slangp',)  # Corrected to be a tuple
    ...
    try:
        ...
        for i in range(3):
            ...
            if check_shader_log(shader):
                ...
                return True
        shader_name = os.path.basename(shader)
        update_status(f"Shader failed to load: {shader_name}", color="red", font=("Press Start 2P", 13, 'normal'))
        return False
    ...
    with open(retroarch_cfg_path, 'w') as file:
        for line in lines:
            if line.startswith('stdin_cmd_enable'):
                ...
                file.write('video_driver = "glcore"\n')
...
def load_core():
    ...
    core = filedialog.askopenfilename(title="Select Core", filetypes=[("Core Files", "*.so")], initialdir="/app/data")
    if core:
        selected_core = core
        core_label.configure(text=f"Selected Core: {get_core_display_name(core)}", text_color="red", font=('Press Start 2P', 10, 'bold'))
        adjust_text_to_fit(core_label)

def load_rom():
    global selected_rom
    rom = filedialog.askopenfilename(title="Select ROM File", initialdir="/app/data")
    if rom:
        selected_rom = rom
        rom_label.configure(text=f"Selected ROM: {os.path.basename(rom)}", text_color="red", font=('Press Start 2P', 10, 'bold'))
        adjust_text_to_fit(rom_label)

def load_bios():
    global selected_bios
    bios = filedialog.askopenfilename(title="Select BIOS File", filetypes=[("BIOS Files", bios_extensions)], initialdir="/app/data")
    if bios:
        selected_bios = bios
        bios_label.configure(text=f"Selected BIOS: {os.path.basename(bios)}", text_color="red", font=('Press Start 2P', 10, 'bold'))
        adjust_text_to_fit(bios_label)

def load_save_state():
    global selected_save_state
    save_state = filedialog.askopenfilename(title="Select Save State File", filetypes=[("Save State Files", "*.state*")], initialdir="/app/data")
    if save_state:
        selected_save_state = save_state
        save_state_label.configure(text=f"Selected Save State: {os.path.basename(save_state)}", text_color="red", font=('Press Start 2P', 10, 'bold'))
...
def handle_crash(slot_number, last_shader, crashed_shaders, retry=False):
    ...
    crash_count += 1
    update_status("C R A S H E D! R E S T A R T I N G !!", color="yellow", font=("Press Start 2P", 13, 'bold italic'))
...
def run_killall():
    """Run the docker kill command to terminate all RetroArch instances."""
    try:
        container_id = subprocess.check_output(["docker", "ps", "-q", "--filter", "name=retroarch-shaders"]).strip()
        subprocess.run(["docker", "kill", "--signal=SIGTERM", container_id], check=True)
...
def finalize_process(shaders, crashed_shaders):
    ...
    crash_report_path = os.path.join("/app/data", "shader_results.txt")  # Save to mounted directory
    ...
    zip_filename = os.path.join("/app/data", f"Shader_results_{current_time}.zip")  # Save to mounted directory
    ...
    update_status(f"Process completed: Downloading to your device.. ", color="red", font=("Press Start 2P", 16, 'normal'))
    quit_retroarch()
    print("RetroArch quit attempt 1")
    ...
    quit_retroarch()
    print("RetroArch quit attempt 2")
    enable_buttons()
    restart_process()
...
def start_retroarch(slot_number=None, last_shader=None, crashed_shaders=None, retry=False):
    ...
    update_status("#Loading libretro CORE-ROM", color="white", font=('Press Start 2P', 10, 'normal'))

    modify_retroarch_cfg()

    # Copy the BIOS file to the designated directory if selected
    if selected_bios:
        update_status("#Copying BIOS file", color="white", font=('Press Start 2P', 10, 'normal'))
        try:
    ...
    if selected_save_state:
        update_status("#Copying Save State", color="white", font=('Press Start 2P', 10, 'normal'))
        corename = get_core_corename(selected_core)
        if corename:
    ...
    try:
        ...
        # Wait for 20 seconds to ensure RetroArch has started
        time.sleep(20)
...
def setup_gui():
    global start_button, pause_button, restart_button, core_listbox, rom_button, bios_button, save_state_button
    global core_label, rom_label, bios_label, save_state_label, status_text, shader_checkboxes
    global select_all_var  # Ensure select_all_var is accessible
    ...
    right_bottom_frame.grid(row=1, column=1, sticky="nsew", padx=10, pady=10)
    ...
    # Initialize select_all_var here
    ...
    def on_select_all():
        ...
        time_estimate = format_time(total_shaders * 35)
    ...
    for shader_subdir in shader_dirs:
        ...
        time_estimate = format_time(len(shaders) * 35)
...
def confirm_start():
    ...
    ctk.CTkLabel(confirm_window, text="CORE:", fg_color="black", text_color="white", font=('Press Start 2P', 12)).pack(
        pady=5)
    ctk.CTkLabel(confirm_window, text=get_core_display_name(selected_core), fg_color="black", text_color="red",
                 font=('Press Start 2P', 12, 'bold')).pack(pady=5)

    ctk.CTkLabel(confirm_window, text="ROM:", fg_color="black", text_color="white", font=('Press Start 2P', 12)).pack(
        pady=5)
    ctk.CTkLabel(confirm_window, text=os.path.basename(selected_rom), fg_color="black", text_color="red",
                 font=('Press Start 2P', 12, 'bold')).pack(pady=5)

    ctk.CTkLabel(confirm_window, text="BIOS:", fg_color="black", text_color="white", font=('Press Start 2P', 12)).pack(
        pady=5)
    ctk.CTkLabel(confirm_window, text=os.path.basename(selected_bios) if selected_bios else "Not selected",
                 fg_color="black", text_color="red", font=('Press Start 2P', 12, 'bold')).pack(pady=5)

    ctk.CTkLabel(confirm_window, text="SAVE STATE:", fg_color="black", text_color="white",
                 font=('Press Start 2P', 12)).pack(pady=5)
    ...
    ctk.CTkButton(confirm_window, text="S T A R T",
                  ...
                  fg_color="white", text_color="black", font=('Press Start 2P', 12, 'bold')).pack(pady=10)
...
def enable_buttons():
    ...
    for checkbox in shader_checkbuttons.values():
        checkbox.configure(state=tk.NORMAL)  # Use the Checkbutton widget, not the BooleanVar
...
def update_fonts():
    ...
    adjusted_font = ('Press Start 2P', font_size)
...
def update_status(message, color="white", font=("Press Start 2P", 13, 'normal')):
