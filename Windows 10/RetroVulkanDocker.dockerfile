...
# Set up Xvfb with custom resolution
ENV DISPLAY=:99
...
RUN apt-get update && \
    ...
    vulkan-tools libvulkan1 mesa-vulkan-drivers x11-xserver-utils \
    ...
    python3-tk libx11-6 libxext6 libxrender1 libxrandr2 libxcursor1 libxfixes3 libpango-1.0-0 libcairo2 xvfb && \
...
# Update the CMD to start Xvfb with the custom resolution
CMD ["sh", "-c", "Xvfb :99 -screen 0 ${RESOLUTION} -ac & \
    export DISPLAY=:99 && \
    sleep 5 && \
    xrandr --fb ${RESOLUTION%x*} && \
    dbus-launch --exit-with-session startxfce4 & \
    cd /opt/dolphin && \
    mkdir -p /home/ralt/Downloads/RetroArch-Linux-x86_64/RetroArch-Linux-x86_64.AppImage.home/system/dolphin-emu && \
    cp -r Data/Sys /home/ralt/Downloads/RetroArch-Linux-x86_64/RetroArch-Linux-x86_64.AppImage.home/system/dolphin-emu && \
    . /opt/venv/bin/activate && \
    python3 /opt/retroarch/RetroVulkan.py"]